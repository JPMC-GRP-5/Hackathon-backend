...
export const generateIternary=async (req,res,next)=>{
    try{
        ...
        if(days>placesByCity.length) {
            res.send(403).send("Not much to explore in these many days, kindly reduce days")
        }
        var budget=req.body.budget;
        const dayWisePlaces=sliceIntoChunks(placesByCity,placesByCity.length/days,budget/days);
